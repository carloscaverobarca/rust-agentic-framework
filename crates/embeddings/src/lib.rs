...
type EmbedFuture<'a> = std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<Vec<f32>>>> + Send + 'a>>;
...
impl EmbeddingProvider for FallbackEmbeddingProvider {
    ...
    fn dimension(&self) -> usize { self.embedding_dimension() }
}

pub async fn create_embedding_provider(cfg: &EmbeddingConfig) -> Result<Box<dyn EmbeddingProvider>> {
    match cfg.provider.as_str() {
        "cohere" => {
            let cohere_cfg = CohereConfig { api_key: std::env::var("COHERE_API_KEY").unwrap_or_default(), ..CohereConfig::default() };
            Ok(Box::new(CohereClient::new(cohere_cfg)?))
        }
        "bedrock-cohere" => {
            let model_id = cfg.model.clone().unwrap_or_else(|| "cohere.embed-multilingual-v3".to_string());
            let aws_region = cfg.aws_region.clone().unwrap_or_else(|| "us-east-1".to_string());
            let br_cfg = BedrockCohereConfig { model_id, aws_region, ..BedrockCohereConfig::default() };
            Ok(Box::new(BedrockCohereClient::new(br_cfg).await?))
        }
        "bedrock-titan" => {
            let model_id = cfg.model.clone().unwrap_or_else(|| "amazon.titan-embed-text-v2:0".to_string());
            let aws_region = cfg.aws_region.clone().unwrap_or_else(|| "us-east-1".to_string());
            let mut br_cfg = BedrockTitanConfig { model_id, aws_region, ..BedrockTitanConfig::default() };
            if let Some(dim) = cfg.dimensions { br_cfg.output_embedding_length = Some(dim as u32); }
